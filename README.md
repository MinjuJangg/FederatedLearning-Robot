...
## 프로젝트 배경
**동국대학교 시뮬레이션 기반 최적화 연구실** 학부 인턴 시절 진행
...
이를 통해 **데이터 이동 없이도** 분산된 현장 데이터를 활용할 수 있었고, 모델의 **일반화 성능**을 개선 가능
--> 본 프로젝트는 각 로봇 별로 다른 번호의 상자를 picking 하는 task를 수행


## 담당 파트: 로봇 컨트롤러
본 레포지토리는 전체 프로젝트 중 **로봇 제어기(Controller) 모듈**만을 포함
...
## 담당 역할
- 전체 프로세스 중 **데이터 수집 및 전처리, 로봇 제어기 설계 및 구현** 담당
